<div class='todo_edit_wrap' id='todo_edit_wrap{{ $task->id }}'>
  ...
  <form class="todo_edit" action="/taskedit/{{ $task->id }}" method="POST" id="todo_edit{{ $task->id }}" data-task-id="{{ $task->id }}">
    ...
    <textarea rows="1" type="text" maxlength="255" name="text" class="form-control" placeholder="Add Description...">{{ $task->text }}</textarea>
    <span id="bulletColor{{ $task->id }}" class="bulletColor">&#8226;</span>
    <select id="category_select{{ $task->id }}" class="category_select" name="category_id">
  ...
  <script>
    ...
    $('#bulletColor{{ $task->id }}').css('color', $("#category_select{{ $task->id }} option:selected").data('categoryColor'));
